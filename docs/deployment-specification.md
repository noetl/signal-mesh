...
| `NOETL_SIGNAL_MESH_FRESHNESS` | *(unset)* = off | no | ⭐ **M3 — the freshness gate.** Arms the check that a tier's fold actually reached the watermark it read at, under the policy below. Only the exact string `true` arms it. ⚠ Armed **separately from the policy**: `fold::admits` shipped with M1 fully tested and with **zero production callers**, so wiring it without an arm would turn a freshness refusal into a new way for a working cascade to fail. |
| `NOETL_SIGNAL_MESH_SEQ_PER_MILLI` | `1` | no | The caller's declared exchange rate between this POC's **sequence** gap and M3's **millisecond** budget. ⚠⚠ The units genuinely differ — `ReadConsistency::Bounded` carries a wall-clock budget and this POC has no clock. There is no correct universal value, which is why it is configuration rather than a constant. |
| `NOETL_EHDB_READ_CONSISTENCY` | `strong` | no | ⚠ **Declared by ehdb, not by this crate** — one name for one concept, read through `ehdb_l0::region_routing::read_consistency_from_env`. `strong` \| `bounded` \| `exact`. ⚠⚠ The binary **refuses to start** on a malformed value (`bounded` with no bound, or an unknown level) rather than defaulting to `strong` — a silent default would serve a policy nobody configured and make an operator's typo look like a working deployment. |
| `NOETL_EHDB_MAX_STALENESS_MS` | `0` | only with `bounded` | ⚠ Declared by ehdb. The wall-clock budget for `bounded`. |
